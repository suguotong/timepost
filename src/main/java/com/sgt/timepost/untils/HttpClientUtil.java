...
public class HttpClientUtil {
    ...
    static {
        ...
        if(StringUtils.isNotEmpty(proxyIP)&&proxyPort>1&&StringUtils.isNotEmpty(scheme)){
            httpHost = new HttpHost(proxyIP,proxyPort,scheme);
        }
        if(httpHost!=null){
            requestConfig =  RequestConfig.custom().setConnectionRequestTimeout(REQUESTCONNECT_TIMEOUT)
                    .setConnectTimeout(CONNECT_TIMEOUT).setSocketTimeout(SOCKET_TIMEOUT).setProxy(httpHost).build();
        }else {
            requestConfig =  RequestConfig.custom().setConnectionRequestTimeout(REQUESTCONNECT_TIMEOUT)
